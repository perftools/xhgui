...
class Xhgui_Db_Mapper
{
    ...
    protected function _conditions($search)
    {
        ...
        if (!empty($search['request_start'])) {
            $conditions['meta.request_ts']['$gte'] = new MongoDate($this->_convertDate($search['request_start']));
        }
        if (!empty($search['request_end'])) {
            $conditions['meta.request_ts']['$lte'] = new MongoDate($this->_convertDate($search['request_end']));
        }
    ...
    protected function _sort($options)
    {
        ...
        if (
            ...
            return array('meta.request_ts' => $direction);
        }
